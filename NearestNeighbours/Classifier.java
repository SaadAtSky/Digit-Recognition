...
public class Classifier {
	...
	public void test1() {
		...
		while(counter<sizeOfDataset) {//iterate through rows in test data
			...
			//iterate through rows in train data to find the closest value to the test value
			for(int row = 1;row<sizeOfDataset;row++) {
				for(int index = 0;index<numberOfFeatures;index++) {
					trainSetValue[index] = dataset2[row][index];
				}
				currentValue = distanceCalculator(testSetValue,trainSetValue);//distance between test value and the current train value
				//keep track of the closest value and use it as the prediction for the test value
	...
	//accuracy % for both data sets and overall average accuracy using this implementation of Nearest Neighbours Algorithm
